...
function formatOutputGrammar(grammar = new Grammar()) {
  console.log(`Grammar: `)
  console.log(`Terminal: ${[...grammar.getTerminator()].toString()}`)
  console.log(`NonTerminal: ${[...grammar.getNonTerminator()].toString()}`)
  console.log(`StartSymbol: ${grammar.getStartSymbol().toString()}`)
  formatOutputProdutions(grammar.getProductions());
}

/**
 *
 * @param {String} production 需要被替换的产生式右部
 * @param {String} n  需要被替换的非终结符
 * @param {String} nProduction 需要替换成的新字符串（n对应的产生式右部）
 */
...
function shuffle(arr = []) {
  ...
  for (let i = cloneArr.length - 1; i > 0; i--) {
    const rd = Math.floor(Math.random() * (cloneArr.length - 1)) + 1;
    temp = cloneArr[i];
